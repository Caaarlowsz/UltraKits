...
public class Events implements Listener
{
    ...
    public void antiMove(final PlayerMoveEvent e) {
        ...
        if ((e.getFrom().getBlockX() != e.getTo().getBlockX() || e.getFrom().getBlockZ() != e.getTo().getBlockZ()) && Events.move.contains(p.getName())) {
                    ...
                    if (i != null) {
                        for (final String s : Main.plugin.getConfig().getStringList("Vitoria")) {
                            if (req.getInventory().contains(Material.MUSHROOM_SOUP)) {
                                i.sendMessage(s.replaceAll("&", "§").replaceAll("@perdedor", p.getName()).replaceAll("@vencedor", req.getName()).replaceAll("@sopas", new StringBuilder().append(this.getAmount(req, Material.MUSHROOM_SOUP)).toString()).replaceAll("@coracoes", new StringBuilder().append((int)req.getHealth() / 2).toString()));
                            }
                            else {
                                i.sendMessage(s.replaceAll("&", "§").replaceAll("@perdedor", p.getName()).replaceAll("@vencedor", req.getName()).replaceAll("@sopas", "0").replaceAll("@coracoes", new StringBuilder().append(req.getHealth() / 2.0).toString()));
                            }
                        }
                    }
                }
                        ...
                        if (i != null) {
                            for (final String s : Main.plugin.getConfig().getStringList("Vitoria")) {
                                if (killer.getInventory().contains(Material.MUSHROOM_SOUP)) {
                                    i.sendMessage(s.replaceAll("&", "§").replaceAll("@perdedor", p.getName()).replaceAll("@vencedor", killer.getName()).replaceAll("@sopas", new StringBuilder().append(this.getAmount(killer, Material.MUSHROOM_SOUP)).toString()).replaceAll("@coracoes", new StringBuilder().append((int)killer.getHealth() / 2).toString()));
                                }
                                else {
                                    i.sendMessage(s.replaceAll("&", "§").replaceAll("@perdedor", p.getName()).replaceAll("@vencedor", killer.getName()).replaceAll("@sopas", "0").replaceAll("@coracoes", new StringBuilder().append((int)killer.getHealth() / 2).toString()));
                                }
                            }
                        }
                    }
    ...
    public void onDropBowl(final PlayerDropItemEvent e) {
        if (Commands.em.contains(e.getPlayer().getName()) && (e.getItemDrop().getItemStack().getType() == Material.BOWL || e.getItemDrop().getItemStack().getType() == Material.MUSHROOM_SOUP)) {

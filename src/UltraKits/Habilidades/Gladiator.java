...
public class Gladiator implements Listener
{
        ...
        if (event.getRightClicked() instanceof Player) {
            ...
            if (p.getItemInHand().getType() == Material.IRON_FENCE && Main.gladiator.contains(p.getName())) {
                ...
                if (this.generateGlass) {
                    ...
                    Main.gladgladiator.add(p.getName());
                    Main.gladgladiator.add(r.getName());
                    ...
                    this.id1 = Bukkit.getScheduler().scheduleSyncDelayedTask(this.plugin, (Runnable)new Runnable() {
                        ...
                        public void run() {
                            if (Gladiator.fighting.containsKey(p.getName()) && Gladiator.fighting.get(p.getName()).equalsIgnoreCase(r.getName()) && Gladiator.fighting.containsKey(r.getName()) && Gladiator.fighting.get(r.getName()).equalsIgnoreCase(p.getName())) {
                                ...
                                Main.gladgladiator.remove(p.getName());
                                Main.gladgladiator.remove(r.getName());
    ...
    public void onBlockBreak(final BlockBreakEvent e) {
        if (e.getBlock().getType() == Material.GLASS && e.getPlayer().getGameMode() != GameMode.CREATIVE && Gladiator.fighting.containsKey(e.getPlayer().getName())) {
            e.setCancelled(true);
    ...
    public void onPlayerLeft(final PlayerQuitEvent e) {
        ...
        if (Gladiator.fighting.containsKey(p.getName())) {
            ...
            Gladiator.fighting.remove(p.getName());
            Main.gladgladiator.remove(p.getName());
            Main.gladgladiator.remove(t.getName());
    ...
    public void onDeathGladiator(final PlayerDeathEvent e) {
        ...
        if (Gladiator.fighting.containsKey(p.getName())) {
            ...
            Gladiator.fighting.remove(p.getName());
            Main.gladgladiator.remove(p.getName());
            Main.gladgladiator.remove(k.getName());
            final Location loc = this.localizacao.get(p);
            final List<Location> cuboid = new ArrayList<Location>();
            cuboid.clear();
            for (int bX = -10; bX <= 10; ++bX) {

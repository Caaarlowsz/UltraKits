...
public class Ninja implements Listener
{
    ...
    public void teleportar(final PlayerToggleSneakEvent e) {
        ...
        if (Main.ninja.contains(p.getName()) && p.isSneaking()) {
            if (Ninja.tp.containsKey(p.getName())) {
                if (!Ninja.cooldown.containsKey(p.getName()) || Ninja.cooldown.get(p.getName()) <= System.currentTimeMillis()) {
                            ...
                            if (!Main.inGladiator(req)) {
                                if (Main.areaPvP(p)) {
                                    if (Main.areaPvP(req)) {
                                        ...
                                        Ninja.cooldown.put(p.getName(), System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(12L));
                                    }
                                    else {
                    ...
                    p.sendMessage(ChatColor.RED + "Faltam " + TimeUnit.MILLISECONDS.toSeconds(Ninja.cooldown.get(p.getName()) - System.currentTimeMillis()) + " segundos para poder usar novamente.");
                }
            }
            else {
    ...
    public static void resetTarget(final Player p) {
        for (final Map.Entry e : Ninja.tp.entrySet()) {

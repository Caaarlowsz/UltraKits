            ...
            if (target == null) {
                ...
                for (int length = (onlinePlayers = Bukkit.getServer().getOnlinePlayers()).length, j = 0; j < length; ++j) {
                    ...
                    if (staff.hasPermission("uk.ver")) {
                        staff.sendMessage("§4[" + Main.plugin.getConfig().getString("ServerName") + "] " + ChatColor.RED + ChatColor.ITALIC + target.getName() + ChatColor.GRAY + " foi reportado por " + ChatColor.RED + ChatColor.ITALIC + sender.getName() + ChatColor.GRAY + "!" + ChatColor.RED + " Motivo: " + ChatColor.GRAY + ChatColor.ITALIC + allArgs);
                    }
                }
            }
        }

...
public class Tag implements CommandExecutor {
	public boolean onCommand(final CommandSender sender, final Command command, final String cmd, final String[] args) {
		...
		if (cmd.equalsIgnoreCase("cor")) {
			...
			if (args.length == 0) {
				p.sendMessage(ChatColor.GOLD + "/cor " + ChatColor.RED + "<cor> " + ChatColor.WHITE + "ou "
						+ ChatColor.GOLD + "/cor " + ChatColor.RED + "<player> <cor>");
				return true;
			}
			if (args.length == 1) {
				try {
				...
				if (target != null) {
					try {
						...
						p.sendMessage(ChatColor.GREEN + "Alterado a cor do nome de " + ChatColor.WHITE
								+ target.getName() + ChatColor.GREEN + " para: " + cor2 + cor2.name());
